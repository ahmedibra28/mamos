...
const Countries = () => {
  ...
  const { getCountries, postCountry, updateCountry, deleteCountry } =
    useCountriesHook({
      page,
      q,
    })
  ...
  const { data, isLoading, isError, error, refetch } = getCountries
...
export default dynamic(() => Promise.resolve(withAuth(Countries)), {
  ssr: false,
})
